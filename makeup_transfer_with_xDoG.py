...
img_source2=cv2.imread('img_normal_2.jpg',cv2.IMREAD_UNCHANGED)
        ...
        for j in range(y,y+h+1):
            for i in range(x,x+w+1):
                ...
                if value!=-1 and a2!=0:
                    ...
                    output[j,i]=(1.0-x2f)*(1.0-y2f)*img_source1_copy[y2i,x2i]+x2f*(1.0-y2f)*img_source1_copy[y2i,x2i+1]
                    output[j, i]=output[j,i]+x2f*y2f*img_source1_copy[y2i+1,x2i+1]+(1.0-x2f)*y2f*img_source1_copy[y2i+1,x2i]
...
img_source2_copy=cv2.cvtColor(img_source2_copy, cv2.COLOR_BGR2GRAY)
subject_l=(XDoG(img_source2_copy, 1.62, 2.7, 13.6, 6.3, 0.927)*255).astype(np.uint8)

# xDoG
subject_lab[:,:,0]=np.copy(subject_l)
subject=cv2.cvtColor(subject_lab, cv2.COLOR_LAB2BGR)

cv2.imshow("image", subject)
cv2.waitKey(0)
cv2.destroyAllWindows()

(example_face,example_detail,example_color_a,example_color_b)=layer_decomposition(example_lab)
...
(subject_face,subject_detail,subject_color_a,subject_color_b)=layer_decomposition(subject_lab)
...
def skin_detail_transfer(src_img, tar_img, delta_i, delta_e):
    ...
    for key in dict.keys():
        if dict[key][0]==1:
            result_detail[key[0]][key[1]] = delta_i * tar_img[key[0]][key[1]]+ delta_e * src_img[key[0]][key[1]]
...
def color_transfer(src_img, tar_img, gamma):
    ...
    for key,value in dict.items():
        if (value[0] != 3):
            result_color[key[0]][key[1]] = (1 - gamma) * tar_img[key[0]][key[1]] + gamma * src_img[key[0]][key[1]]
        else:
...
def lip_transfer(src_img, tar_img, sigma):
    ...
    for key1 in dict_lip.keys():
        ...
        for key2 in dict_lip.keys():
            ...
            gaussian_val = gaussian(distance, sigma) * gaussian(intensity_diff, sigma)
...
result_color_a=result_color_a.reshape(result_color_a.shape[0],result_color_a.shape[1],1)
result_color_b=result_color_b.reshape(result_color_b.shape[0],result_color_b.shape[1],1)
